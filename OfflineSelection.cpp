...
int main(int argc, char** argv){
  ...
  TFile *fOut = TFile::Open(Form("%sVBFHTauTau_tau25tau25_jet30_pTtautau100.root", LLRdir.Data()),"recreate");
  ...
  TFile *fPlots = TFile::Open(Form("%sVBFHTauTau_tau20tau20_jet30_plots.root", LLRdir.Data()),"recreate");
  fPlots->cd() ;  
  ...
  for (long int iEv = 0; iEv < nEvents; iEv++){
	...
	if(tlv_tau.Pt()>20 && fabs(tlv_tau.Eta())<2.1 && ((tauID->at(i)>>5)&1) && ((tauID->at(i)>>7)&1) && ((tauID->at(i)>>5)&11)){
	...
	if(tlv_jet.Pt()>30&&tlv_jet.DeltaR(OffTau[0])>0.3 && tlv_jet.DeltaR(OffTau[1])>0.3 && tlv_tauPair.Pt()>100){ //overlap removal offline
	...
	if (L1_DoubleIsoTau25er_Jet50 && OffJet[0].Pt()>60) n_ditaujet +=1;
  ...
  fPlots->Write();
  fPlots->Close();
  cout<<"Events passing L1_DoubleIsoTau32er                                  "<<acceptance_ditau<<endl;
  cout<<"Events passing L1_DoubleIsoTau25er_PtTauTau70                       "<<n_ditaujet<<endl;
