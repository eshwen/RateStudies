...
int main(int argc, char** argv){
  ...
  if(LLRframework){
    ...
    tInput->SetBranchAddress("bCSVscore", &bCSVscore, &b_bCSVscore);
  ...
  std::vector <TLorentzVector> OffBJetNoOverlap_DiTau;
  std::vector <TLorentzVector> OffBJetNoOverlap_VBF;
  std::vector <TLorentzVector> OffBJet;
  ...
  std::vector< std::pair<int, float> > bjet_CSV;
  std::vector< std::pair<int, float> > bjet_CSV_DiTau;
  std::vector< std::pair<int, float> > bjet_CSV_VBF;   
  ...
  for (long int iEv = 0; true ; iEv++){
	...
	if(tlv_jet.Pt()>pt_bjet_cut){
      ...
      std::sort(bjet_CSV.begin(), bjet_CSV.end(), SortJetByCSV);
