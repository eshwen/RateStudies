...
if __name__ == "__main__":
    ...
    parser.add_option('-c', '--cfg'           ,    dest='cfg'                , help='cfg file'                                           , default='L1ntuples_T3.py')
    ...
    relativeJobDir = 'JobLauncher_%s_%dEvents'%(opt.nameTag, opt.maxevents)
    jobsDir=cmsswBase+'/src/RateStudies/L1ntuples/test/' + relativeJobDir
    ...
    if not outFullPath.endswith('/'):
        outFullPath += '/'
    outFullPath += 'L1NtuplesOutput_%s_%dEvents/'%(opt.nameTag, opt.maxevents)
    ...
    for n in xrange (0, nloop) :
        ...
        outFullLFNNtuples  = outFullLFN + 'L1ntuples_%d.root' % n
